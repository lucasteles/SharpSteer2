...
/// </summary>
public class SphericalObstacle(float r, Vector3 c) : IObstacle
{
    public Vector3 Center = c;
    ...
    public Vector3 SteerToAvoid(IVehicle v, float minTimeToCollision)
    {
        ...
        var avoidance = Vector3Helpers.PerpendicularComponent(v.Position - Center, v.Forward);
    ...
    public float? NextIntersection(IVehicle vehicle)
    {
        ...
        var lc = vehicle.LocalizePosition(Center);

        // computer line-sphere intersection parameters
        var b = -2 * lc.Z;
        var totalRadius = Radius + vehicle.Radius;
        var c = (lc.X * lc.X) + (lc.Y * lc.Y) + (lc.Z * lc.Z) - (totalRadius * totalRadius);
        var d = (b * b) - (4 * c);
